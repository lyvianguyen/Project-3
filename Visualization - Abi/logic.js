  ...
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    d3.json(url).then (function(data){
       for(i=0; i<data.length;i++){
        if(data[i]['state']!="United States"){
        death_toll.push(parseInt(data[i]['death']));
        AADR.push(data[i]['AADR']);
    }
        for(j=0; j<statesJSON['features'].length;j++){
            if(statesJSON['features'][j]['properties']['NAME']===data[i]['state']){
                statesJSON['features'][j]['properties']['Death_Toll']=data[i]['death'];
                statesJSON['features'][j]['properties']['Age_Adjusted_Death_Rate']=data[i]['AADR'];
            }
       ...
       let low=0;
       let step = 0;
       if(death.value==="Death Toll"){
        low = Math.min(...death_toll);
        step = (Math.max(...death_toll)-Math.min(...death_toll))/8;
       }
       else{
        low = Math.min(...AADR);
        step = (Math.max(...AADR)-Math.min(...AADR))/8;

       }
        function getColor(d) {
            return colors[Math.ceil((d-low)/step)-1];
        }
        ...
        function style(features) {
           if (death.value==="Death Toll"){
            return {
                fillColor: getColor(features.properties.Death_Toll),
                weight: 2,
                opacity: 1,
                color: 'white',
                dashArray: '3',
            ...
            return {
                fillColor: getColor(features.properties.Age_Adjusted_Death_Rate),
                weight: 2,
                opacity: 1,
                color: 'white',
                dashArray: '3',
        ...
        // var legend = L.control();
          ...
          legend.onAdd = function(map) {
            ...
            grades = [];
            for (i =0; i<8; i++){
                grades.push(parseInt(low+i*step));
            }
            ...
            for (var i = 0; i < grades.length; i++) {
             if(i<7)
              div.innerHTML +=('<i style="background:' + getColor(grades[i] ) + '"></i>'+grades[i] +'&ndash;'+ grades[i + 1]+'<p>');
            else
        ...
        info.update = function (props) {
            this._div.innerHTML = '<h4>Death Info Per State</h4>' +  (props ?
                '<b>' + props.NAME + '</b><br />' + props.Death_Toll + ' death toll<br/>'+props.Age_Adjusted_Death_Rate+' Age-adjusted death rate'
                : 'Hover over a state');
        };
        ...
        L.geoJson(statesJSON, {style: style, onEachFeature: onEachFeature}).addTo(myMap);
...
let myMap = L.map("map", {
  ...
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
   ...
   L.geoJSON(statesJSON).addTo(myMap);
